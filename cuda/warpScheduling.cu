...
void DisplayHeader()
{
  ...
  __device__ void recursively_divergent(int *a, int* w, int *k) {
  ...
  for(int i=0; i<R*C*H; i++)
     ...
     if (gtid != (R*C*H - 1) && gtid != k[1])
        ...
        {   
          // __syncthreads();
         a[gtid] = (atomicAdd(&k[1],1));
           w[gtid] = gtid/32;
        }   
      recursively_divergent(a,w,k);
     }   
